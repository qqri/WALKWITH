...
public class WalkService {
    ...
    @Transactional
    public UserExerVO updateExer( UserExerVO userExerVO ){
    ...
    @Transactional
    public ResponseVO attackFriend(FriendAttackVO friendAttackVO) {
        ...
        if(check != null) {//알람이 이미 존재하는지 확인함. > null 이 아니란건 이미 존재한단 거니까 끝내버림 > 이 경우엔 아예 공격갔다는 말도 안뜸
            return  new ResponseVO("warning : already attack");
        }
    ...
    */
    @Transactional
    public User initToday(UserInfoVO userInfoVO) {
